import { w as I, r, u as y, j as s } from './chunk-PVWAREVJ-DHjQzAty.js';
import { E as M, F as P, a as U, g as v } from './modal-BhtBJdvC.js';
import { l as A } from './firebase-yRvBWTGt.js';
import { B as D } from './button-Bj7XQSOb.js';
import { u as O } from './AuthContext-Cy9VIbgu.js';
import { u as T } from './useTranslation-ZsYxALZe.js';
import { F as B } from './index.esm-DEVYYNpo.js';
import './i18next-Bhq85MQn.js';
import './x-D9UkY--3.js';
import './index-BzYe63nO.js';
import './index-Cd_xhWcG.js';
const X = I(function () {
  const { t: c } = T(),
    [o, b] = r.useState({ username: '', email: '', password: '' }),
    [u, m] = r.useState({}),
    [, j] = r.useState(!1),
    { user: p, token: h, setUser: k, setToken: E } = O(),
    f = y(),
    [N, w] = r.useState(!1),
    [S, C] = r.useState(''),
    d = (l) => {
  ...
  const g = (l) => {
      const { name: t, value: i } = l.target;
      b((n) => ({ ...n, [t]: i }));
      const a = v().safeParse({ ...o, [t]: i });
      if (a.success) m((n) => ({ ...n, [t]: void 0 }));
      else {
        const n = a.error.issues.find((x) => x.path[0] === t);
        m((x) => ({ ...x, [t]: n ? n.message : void 0 }));
      }
    },
    F = async (l) => {
      (l.preventDefault(), j(!0));
      const i = v().safeParse(o);
      if (i.success) {
        m({});
        try {
          const e = await A(o.email, o.password);
          e &&
            (console.log('is logined:', e.user),
  ...
  return (
    r.useEffect(() => {
      (console.log('user from context:', p),
        console.log('token from context:', h));
    }, [p, h]),
    s.jsxs('form', {
      onSubmit: F,
      className: 'form-position  text-purple-600  flex flex-col items-center',
      children: [
        s.jsx(M, { isOpen: N, onClose: () => w(!1), message: S }),
        s.jsx('h2', {
